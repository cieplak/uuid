...
int main(int argc, char **argv)
{
    ...
    if (argc == 1) {
        ...
        // Check for endianness setting
        auto little_endian_arg = std::find(args.begin(), args.end(), "--end");
        if (little_endian_arg != args.end()) {
            endianness = UUID::Endianness::Little;
            args.erase(little_endian_arg);
        } else {
            endianness = UUID::Endianness::Big;
        }
        auto argument = boost::algorithm::join(args, " ");

        if (strcmp(command, "encode") == 0) {
            auto id = UUID::parse(argument);
            std::cout << UUID::base58_encode(id, endianness) << std::endl;
        } else if (strcmp(command, "decode") == 0) {
            auto id = UUID::base58_decode(argument, endianness);
            std::cout << UUID::serialize(id) << std::endl;
        } else {
            std::cout << "Unknown command: " << command << std::endl;
